...
<% 
    ...
    ArrayList arrNum = ViewData["bidNum"] as ArrayList;  //出价次数、参与人数、围观人数

    //已中标状态（等待公布） 已成交状态（活动结束）
    List<TXOrm.BidPrice> _bidPrice = (ViewData["BidPrice"] as List<TXOrm.BidPrice>).Where(b => b.Status > 0).OrderByDescending(b => b.Status).ThenByDescending(b => b.Id).ToList();
%>
<%
    //活动未开始
    if (_activity.ActivitieState == 0)
    {
%>
<%
        //1 显示“即将开始”
...
<div class="w1190 mb15">
    <div class="yellow_box1 r_y_kj clearFix">
        <div class="part1">
        ...
        <div class="part2">
            <table width="100%" cellspacing="0" cellpadding="0" border="0">
                <tr>
                    ...
                    <td>
                        <strong class="green mr3">
                            <%=(double)Model.TotalPrice %></strong>万元<span class="red">[封顶价格]</span>
                    </td>
                    <th align="right" scope="row">
                        竞购方式：
                    </th>
                    <td>
                        砍价 <span class="r_wen_green ml20"><i class="r_layer_box">在规定时间内开发商提供一定数量的房源并设置底价，用户出价最低且唯一则视为竞购成功。
                ...
                <tr>
                    ...
                    <td>
                        <strong class="green mr3">
                            <%=(double)_activity.BidPrice%></strong>万元-<strong class="green mr3"><%=(double)Model.TotalPrice%></strong>万元
                    </td>
                </tr>
                <tr>
                    <th align="right" scope="row">
                    ...
                    <td colspan="3">
                        单次减价不高于<span class="green mlr3"><%=(double)_activity.MaxPrice%></span>万元
                    </td>
                </tr>
            </table>
        </div>
...
<%
    //活动期间
    else if (_activity.ActivitieState == 1)
    {
%>
<%
        //1 显示“倒计时”
...
<div class="w1190 mb15">
    <div class="yellow_box1 r_y_kj clearFix">
        ...
        System.TimeSpan ts = endTime.Subtract(DateTime.Now);
        string miaoshaSeconds = ts.TotalSeconds.ToString();
        %>
        <div class="part3">
            <p class="clock">
                竞购时间</p>
            <p class="col333 font14">
                ...
                -
                <%=_activity.EndTime.ToString("yyyy-MM-dd") %></p>
            <p class="col999 font14" id="P1" lang="<%=Model.ActivityState  %>" data="<%=miaoshaSeconds %>">
...
<div class="w1190">
    <a name="chujia"></a>
    <div class="r_title_lp">
        <strong class="title_span pad_0_30">砍价</strong><span class="col666 ml20 mr15">买家出价:<i
    ...
    <div class="gray_box clearFix">
        ...
        if (!(bool)ViewData["IsLogin"])
        {
            ...
            <p>
                ...
                <strong class="red">请先登录再进行报名！</strong>
            </p>
        ...
        else
        {
            ...
            decimal lastPrice = _lastbidPrice != null ? _lastbidPrice.BidUserPrice : Model.TotalPrice; 
            %>
                        ...
                        submitHandler: function (form) {
                            ...
                            $.post(globalvar.siteRoot + "/ajax/GetLastBidPrice", { activityId: activitiesId, houseId: houseId }, function (d) {
                                ...
                                if (parseFloat(price) >= lastPrice) {
                                    errMsg.push('<li>出价不能高于上次出价（上次出价' + lastPrice + '万元）</li>');
                                }
                                if (parseFloat(lastPrice - parseFloat(price)) % parseFloat('<%=_activity.AddPrice %>') != 0) {
                                    errMsg.push('<li>减价幅度必须是<%=(double)_activity.AddPrice %>万元的整数倍</li>');
                                }
                                if (lastPrice - parseFloat(price) > parseFloat('<%=_activity.MaxPrice %>')) {
                                    errMsg.push('<li>单次减价不能高于<%=(double)_activity.MaxPrice %>万元</li>');
                                }
                                ...
                                $.ajax({
                                    ...
                                    dataType: "json",
                                    data: { updatesalesstatus: 0, status: 0, bidtype: 2, price: price, realName: realName, code: code, mobile: mobile, qq: qq, email: email, activitiesId: activitiesId, developerId: developerId, houseId: houseId, type: 5 },
                                    beforeSend: function (XMLHttpRequest) {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    ...
                                    if (result == 1) {
                                        ...
                                        globalvar.dialog("出价失败！", function () { window.location.reload(); });
                                    }
                                }
                            });
                        },
...
<%
    //活动结束
    else if (_activity.ActivitieState == 2)
    {
        //1 显示“房源详情”
        Html.RenderPartial("~/Views/Shared/_HouseDetail.ascx", Model);
%>
<div class="w1190">
    ...
    <div class="r_title_lp">
        <strong class="title_span pad_0_30">砍价</strong><span class="col666 font12 ml20 mr15">买家出价:<i
    ...
    <div class="yellow_box1 yellow_box2 clearFix mb15">
        ...
        <%
        if (_bidPrice.Count > 0 && _bidPrice[0].Status == 2)
        {
        %>
...
%>
<%
    var _theLastBid = new BidPrice();
    if (_bidPrice.Count > 0)
    {
        _theLastBid = _bidPrice.FirstOrDefault();
    }
%>
        ...
        $.post("<%=TXCommons.GetConfig.GetSiteRoot%>/Premises/BidPricesPage", { m: Math.random(), activityId: activityId, houseId: houseId, pageIndex: index, type: type1 }, function (d) {
            if (d != "0") {
                ...
                for (var i = 0; i < RevList.length; i++) {
                    var item = RevList[i];
                    if (type1 == 3) {
                        tval = "<td><span " + (item.Status == 0 ? "" : "class='yellow_bjtb'") + ">" + (item.Status == 0 ? ('<%=(_theLastBid.Status == 2?"出局":"") %>') : (item.Status == 1 ? "中标" : "成交")) + "</span></td>";
                    }
