<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<TXOrm.Premis>" %>
 <div class="map_left">
    <div    id="mapContainer"  style="width: 997px; height: 890px;" >
    </div>
</div>
    ...
    var PremisesMap =
        {
            ...
            pageCreate: function () {
                ...
                pagestr += map_pageIndex > 1 ? "<a href=\"javascript:local.gotoPage(" + (pre - 1) + ");\">&lt;&lt;</a>" : "<em class=\"noclick\">&lt;&lt;</em>";
                for (i = startcount; i <= endcount; i++) {
                    pagestr += map_pageIndex == i ? "<a href=\"javascript:;\" class=\"no\">" + i + "</a>" : "<a href=\"javascript:local.gotoPage(" + (i - 1) + ");\">" + i + "</a>";
                }
                pagestr += map_pageIndex != allpage ? "<a href=\"javascript:local.gotoPage(" + (next - 1) + ");\">&gt;&gt;</a>" : "<em class=\"noclick\">&gt;&gt;</em>";
                pagestr += "</div>";
                return pagestr;
            }
        }
...
<script type="text/javascript">
    $(function () {
        ...
        $.ajax({
            type: "get",
            url: "/xinfang/MapSearchPremises/GetAsyPremisesInfo",
            data: { id: "<%=Model.Name%>", m: Math.random() },
            cache: false,
            async: false,
            dataType: "json",
            success: function (result) {
                if (result) {
                    if (result.icount > 0) {
                        var saddress = result.address.split("##");
                        var stitle = result.title.split("##");
                        var sLng = result.Lng.split("##");
                        var sLat = result.Lat.split("##");
                        for (i = 1; i <= result.icount - 1; i++) {
                            var map_address = saddress[i];
                            var map_title = stitle[i];
//                            alert(sLng[i]);
//                            alert(sLat[i]);
                            var map_point = new BMap.Point(sLng[i], sLat[i]);
