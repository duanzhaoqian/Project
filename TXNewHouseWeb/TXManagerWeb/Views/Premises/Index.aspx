...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="data">
        ...
        <div class="filterBar">
            <% using (Html.BeginForm("search", "Premises", FormMethod.Post, new { Id = "searchForm", Name = "searchForm" }))
               {%>
    ...
    <script type="text/javascript">
        ...
        function pageselectCallback(page_index, jq) {
            ...
            $("#PremisesResult").load('<%=Url.SiteUrl().Premises_SearchResult("searchresult",Model)%>?DeveloperName=<%=Model.DeveloperName %>&PremisesName=<%=Model.PremisesName %>&pageindex=' + page_index + '&pagesize=' + opts.items_per_page);
        }
        //初始化分页参数
        ...
        $(document).ready(function () {
            $.post('<%=Url.SiteUrl().Premises_SearchResult("searchcount",Model)%>?DeveloperName=<%=Model.DeveloperName %>&PremisesName=<%=Model.PremisesName %>', function (data, textStatus) {
    ...
    <script type="text/javascript">
        function delpremises(pid, obj) {
            if (confirm("确定删除该楼盘？")) {
                $.ajax({
                    url: "<%=Auxiliary.Instance.NhManagerUrl %>Premises/DelPremises.html",
                    type: "POST",
                    dataType: "json",
                    data: { id: pid },
