        ...
        <div class="outer">
            ...
            <div class="choose mt15">
                <% string tUrl = Auxiliary.Instance.NhManagerUrl + "premises/premisesimage/" + Model.Id + "/0/" + (new Random().Next(9999, 999999)) + ".html?tage=";
                   string tUrll0 = Auxiliary.Instance.NhManagerUrl + "premises/premisesimage10/" + Model.Id + "/0/" + (new Random().Next(9999, 999999)) + ".html?tage="; %>
                <span><a id="t0" href="<%=tUrl + "0" %>">规划图</a> <a id="t1" href="<%=tUrl + "1" %>">
                    交通图</a> <a id="t2" href="<%=tUrl + "2" %>">位置图</a> <a id="t3" href="<%=tUrl + "3" %>">
                        实景图</a> <a id="t4" href="<%=tUrl + "4" %>">效果图</a> <a id="t5" href="<%=tUrl + "5" %>">
                            户型图</a> <a id="t6" href="<%=tUrl + "6" %>">楼栋平面图</a> <a id="t10" href="<%=tUrll0 + "10" %>">
                                楼盘封面广告图</a></span>
            </div>
            <div class="loubox clearFix">
                ...
                <ul id='ul_box'>
                    ...
                    <li id="imageBox_<%= info.ID %>">
                        <dl>
                            <dt>
                                <img src="<%= info.Path + ".180_130.jpg" %>" width="180" height="130" alt="图片" />
                            ...
                            <dd>
                                <span class="red mr5">*</span><input type="text" id="txt_<%= info.ID %>" value='<%= info.Title %>' maxlength="15" placeholder="标题" />
                                <div id="err_title_<%= info.ID %>" style="display: none;" class="tisbox">
                                    <em>&nbsp;</em>请输入标题。</div>
                                <textarea id="des_<%= info.ID %>" placeholder="请填写照片描述"><%= info.Desc %></textarea>
                                <div id="err_des_<%= info.ID %>" style="display: none;" class="tisbox">
                                    <em>&nbsp;</em></div>
                            </dd>
                        </dl>
                    </li>
    ...
    <script type="text/javascript">
        ...
        var premisesimgs = {
            ...
            addUploadPicToImgDiv: function(data) {
                ...
                var html_RoomType = "";
                // 户型图(设置主力户型)
                if (premisesimgs.tagnum == 5) {
                    html_RoomType = "<a style=\"cursor:pointer;\" onclick=\"premisesimgs.setIsForce('" + id + "')\" id=\"hidzla_" + id + "\" class=\"shetxt\">设置为主力户型</a><input id=\"hidzl_" + id + "\" name=\"hidzl_" + id + "\" type=\"hidden\" value=\"0\" />";
                }

                var html = "";
                html += "<li id=\"imageBox_" + id + "\">";
                html += "<dl>";
                html += "<dt><img src=\"" + picUrl + ".180_130.jpg\" width=\"180\" height=\"130\" alt=\"图片\" />";
                ...
                html += "<textarea id=\"des_" + id + "\" placeholder=\"请填写照片描述\"></textarea>";
                html += "<div id=\"err_des_" + id + "\" style=\"display:none;\" class=\"tisbox\"><em>&nbsp;</em></div>";
                html += "</dd>";
                html += "</dl>";
                html += "</li>";
            ...
            valid_uploadpic: function() {
                ...
                for (i = 0; i < arrPic.length; i++) {
                    id = $(arrPic[i]).attr("id").split('_')[1];
                    var t = $.trim($("#des_" + id).val());
                    if (500 < t.length) {
                        $("#err_des_" + id).show();
                        $("#err_des_" + id).html("<em>&nbsp;</em>图片描述最多可输入500个汉字");
                        flag = false;
                    } else {
                        $("#err_des_" + id).hide();
                        $("#err_des_" + id).html("");
...
//                var count;
//                if (premisesimgs.tagnum == 4) {
//                    count = premisesimgs.getUploadedPicsCount();
//                    if (count < premisesimgs.minuploadcount_type4) {
//                        alert('最少上传' + premisesimgs.minuploadcount_type4 + '张图片');
//                        return false;
//                    }
//                } else if (premisesimgs.tagnum == 5 || premisesimgs.tagnum == 6) {
//                    count = premisesimgs.getUploadedPicsCount();
//                    if (count < premisesimgs.minuploadcount) {
//                        alert('最少上传' + premisesimgs.minuploadcount + '张图片');
//                        return false;
//                    }
//                }
...
//                return true;
            ...
            delPic: function(id) {
                ...
                if (5 == premisesimgs.tagnum || 6 == premisesimgs.tagnum) {
                    var url = "";

                    if (5 == premisesimgs.tagnum) {
                        url = '<%= Auxiliary.Instance.NhManagerUrl %>Premises/GetROOMTYPEImageUseCount.html?id=' + id;
                    }
                    if (6 == premisesimgs.tagnum) {
                        url = '<%= Auxiliary.Instance.NhManagerUrl %>Premises/GetPlaneImageUseCount.html?id=' + id;
                    }
                    ...
                    if (0 < result) {
                        if (!confirm("当前图片被使用中，如删除后则图片不能显示。")) {
                            return;
                        }
                    }
                    ...
                    success: function(msg) {
                        ...
                        for (i = 0; i < msg.length; i++) {
                            if (msg[i].result == "true") {
                                $("#imageBox_" + id).remove();
                                premisesimgs.ajaxDelPicFromDb(id);
            ...
            save: function() {
                ...
                $.post(url, ajaxdata, function(msg) {
                    ...
                    if (!msg.suc) {
                        ...
                        alert("保存成功");
                        window.location.href = '<%= Auxiliary.Instance.NhManagerUrl %>premises/premisesimage/<%= Model.Id %>/0/<%= new Random().Next(9999,999999) %>.html?tage=' + premisesimgs.tagnum;
                    }
                });
            }
        };
