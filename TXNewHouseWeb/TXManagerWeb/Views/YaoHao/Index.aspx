            ...
            <% using (Html.BeginForm("Search", "YaoHao", FormMethod.Get, new { Id = "searchForm", Name = "searchForm" }))
               {%>
                   <span style="font-size:12px;">城市：</span> 
                       ...
                       <span style="font-size:12px;">活动名称</span><input type="text" value="<%= Model.Name %>" name="Name" /> 
                      <span style="font-size:12px;">时间 </span> <input type="text" name="BeginTime" value="<%=Model.BeginTime %>" class="Wdate" id="d4321" onFocus="WdatePicker({dateFmt:'yyyy-M-d H:mm:ss',maxDate:'#F{$dp.$D(\'d4322\',{d:-0});}'})" readonly/>
                      -<input type="text" name="EndTime"  value="<%=Model.EndTime %>" class="Wdate" id="d4322" onFocus="WdatePicker({dateFmt:'yyyy-M-d H:mm:ss',minDate:'#F{$dp.$D(\'d4321\',{d:0});}'})" readonly/>
                       <span style="font-size:12px;">状态</span> <%= Html.DropDownListFor(model => model.ActivitieState, Model.ActivitieStateList, new {@class = "w100"}) %>
               ...
               <%}%> 
                ...
                <div class="C">
                    <a href="<%= Url.SiteUrl().YaoHaoIndex(1) %>">摇号活动审批</a></div>
                <div class="R">
                </div>
            </div>
        </div>
