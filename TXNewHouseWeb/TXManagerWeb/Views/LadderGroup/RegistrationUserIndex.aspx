...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
     ...
     <div class="current">
        ...
        <span><a href="javascript:void(0);">
            ...
            <i>&gt;</i> <a href="javascript:void(0);">
                <%=AdminPageInfo.ItemName %></a>-阶梯团购报名用户列表</span>
    </div>
    
     <!--//current-->
     <div class="data">
        <!-- InstanceBeginEditable name="EditRegion3" -->
        <% using (Html.BeginForm("RegistrationUserSearch", "LadderGroup", FormMethod.Post, new { Id = "searchForm", Name = "searchForm" }))
           { %>
        <div class="filterBar" style="display:block;">
            ...
            <div class="L"></div>
            <div class="C"><a href="<%=Url.SiteUrl().ActionUrl("ExportExcel","LadderGroup") %>?id=<%=Model.Id %>" id="btn_output1">导出此列表</a></div>
            <div class="R"></div>
        </div>
        </div>
     ...
     <script type="text/javascript">
        ...
        function pageselectCallback(page_index, jq) {
            ...
            $("#ListResult").load("<%=Url.SiteUrl().RegistrationUser_Search("RegistrationUserSearchResult")%>?Id=<%=Model.Id %>&pageindex=" + page_index + "&pagesize=" + opts.items_per_page);
        }
        //初始化分页参数
        ...
        $(document).ready(function () {
            $.post("<%=Url.SiteUrl().RegistrationUser_Search("RegistrationUserSearchCount")%>?Id=<%=Model.Id %>", function (data, textStatus) {
