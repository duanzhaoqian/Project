...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
        ...
        <span><a href="javascript:void(0);">
            ...
            <i>&gt;</i> <a href="javascript:void(0);">
                <%=AdminPageInfo.ItemName %></a><i>&gt;</i><a href="javascript:void(0);">修改
                    <%=Model.PremisesName +" "+ Model.BuildingName+Model.NameType+" "+Model.UnitName+" "+Model.Floor+"层 "+Model.HouseNum %>一口价活动</a></span>
    </div>
    <%--Current Content--%>
    <div class="data">
            ...
            <div class="tab1">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tb1">
                    <tr>
                        ...
                        <td width="92%">
                            <%=Html.TextBox("BidPrice", Convert.ToDouble(Model.BidPrice))%>万元
                        </td>
                    </tr>
                    <tr>
                        <th width="8%">
                        ...
                        <td width="92%">
                            <%=Html.TextBox("Bond", Convert.ToDouble(Model.Bond))%>元
                        </td>
                    </tr>
                    <tr>
                        <th width="12%">
    ...
    var BidingModify = {
        ...
        LoadData: function (pageindex) {
            ...
            $("#ListResult").load('<%=Url.SiteUrl().APriceUser_Search("housesearchresult")%>?&pageindex=' + pageindex + BidingModify.Url(), function () {
        ...
        PostCount: function () {
            ...
            //请求数据 并生产分页链接
            $.post(
                '<%=Url.SiteUrl().APriceUser_Search("housesearchcount")%>',
                BidingModify.Data(),
                function (data) {
        ...
        Modify: function () {
            $.post("<%=Url.SiteUrl().APriceUser_Search("Modify")%>", BidingModify.ModifyDate(), function (result) {
                if (result.result) {
                    alert("修改成功");
