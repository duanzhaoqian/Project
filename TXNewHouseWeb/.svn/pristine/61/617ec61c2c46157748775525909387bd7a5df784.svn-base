...
{
    public class Bargain_AdminDal:BaseDal_Admin
    {
        #region 砍价活动信息列表
                    ...
                    string sql = string.Format(@"
                       SELECT * FROM (
                                       SELECT  ROW_NUMBER() OVER ( ORDER BY active.CreateTime DESC ) AS RowID,
				                         ...
				                         WHERE active.IsDel=0 AND deve.IsDel=0 AND ah.IsDel=0 AND house.IsDel=0 AND build.IsDel=0 AND premise.IsDel=0 AND Unit.IsDel=0  {0}
...
(select count(1) from dbo.BidPrice where IsDel=0 and (Status=2) and ActivitiesId={3} and HouseId={4})AS HaveWinCount              
        ...
        #region 获取砍价报名用户列表sqlwhere语句

        /// <summary>
        /// 获取砍价报名用户列表sqlwhere语句
        /// </summary>
...
";
        ...
        /// <returns></returns>
        public bool EditBidPriceState(int activeId,int status)
        {
            try
            {
                using (var kyj = new kyj_NewHouseDBEntities())
                {
                    var query = kyj.BidPrices.FirstOrDefault(it => it.ActivitiesId == activeId && it.IsDel == false);
                    var flag = false;
                    if (query != null)
                    {
                        query.Status = status;
            ...
            {
                //记录日志
                Log4netService.RecordLog.RecordException("wangzhikun", activeId, ex);
                return false;
                throw;
            }
        }
        ...
        /// <returns></returns>
        public bool UpdateBargainState_Admin_ZhongBiao(int bid)
        {
            try
            {
                using (var db = new kyj_NewHouseDBEntities())
...
END CATCH", bid);
        ...
        /// <param name="bid"></param>
        /// <returns></returns>
        public bool UpdateBargainState_Admin_CancelZhongBiao(int bid)
        {
            try
            {
                using (var db = new kyj_NewHouseDBEntities())
...
BEGIN TRY
    ...
    IF @MinPrice <> 0 
        BEGIN
            ...
            WHERE   ActivitiesId = @tActivityId
                    AND HouseId = @tHouseId
                    AND BidUserId = (SELECT BidUserId FROM dbo.BidPrice WHERE BidUserPrice = @MinPrice AND ActivitiesId = @tActivityId AND IsDel = 0)
        END
    COMMIT TRAN
    SELECT  '1' AS Code, '操作成功' AS Msg
END TRY
...
", aid);
        ...
        public object GetEntity_ById(int id)
        {
            try
            {
                using (var db = new kyj_NewHouseDBEntities())
                {
                    return db.Activities.FirstOrDefault(active => active.Id == id && active.Type == 6 && active.IsDel == false);
                }
            }
            catch (Exception ex)
            {
            ...
            try
            {
                using (var kyjEnt = new kyj_NewHouseDBEntities())
                {
                       ...
                       string sql =string.Format(@"SELECT
                                      (SELECT MAX(bd.Id) FROM   
                                            (SELECT MAX(Id) AS Id,BidUserId,ActivitiesId  FROM dbo.BidPrice 
                                                GROUP BY BidUserId,ActivitiesId) Man
                                                JOIN dbo.BidPrice bd ON bd.Id=Man.Id
                                                WHERE bd.IsDel=0 AND Man.ActivitiesId={0} ) AS Result", ActivitiesId);
                       ObjectResult<SqlResultScalarInt> query = kyjEnt.ExecuteStoreQuery<SqlResultScalarInt>(sql);
                       List<SqlResultScalarInt> resultlist = query.ToList();
                       if (resultlist.Any())
                           return Convert.ToInt32(resultlist[0].Result);
                       return 0;
