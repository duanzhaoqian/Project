<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/TXNewHouseManager.Master" Inherits="WebRentViewPage<TXModel.AdminPVM.PVS_NH_LadderBuy>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
     ...
     <div class="data">
        ...
        <% using (Html.BeginForm("search", "LadderGroup", FormMethod.Get, new { Id = "searchForm", Name = "searchForm" }))
           { %>
        <div class="filterBar">
     ...
     <script type="text/javascript">
        ...
        function pageselectCallback(page_index, jq) {
            ...
            $("#ListResult").load("<%=Url.SiteUrl().LadderGroup_SearchResult("searchresult",Model)%>?DeveloperName=<%=Model.DeveloperName %>&pageindex=" + page_index + "&pagesize=" + opts.items_per_page);
        }
        //初始化分页参数
        ...
        $(document).ready(function () {
            $.post("<%=Url.SiteUrl().LadderGroup_SearchResult("searchcount",Model)%>?DeveloperName=<%=Model.DeveloperName %>", function (data, textStatus) {
        ...
        var tuangou= {
            ...
            del: function(id) {
                ...
                var url = '<%= Auxiliary.Instance.NhManagerUrl %>laddergroup/del.html';
                var data = { id: id };
