<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Main.Master" Inherits="System.Web.Mvc.ViewPage<List<TXModel.NHouseActivies.Bargain.CT_UserList>>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <div class="content">
        <h4 class="title_h4 mt15">
            <span>
                <%=ViewData["Title"] %>-砍价用户列表 </span>
            <div class="font14 fr mr35 mt10">
                <a href="<%=TXCommons.GetConfig.BaseUrl%>Activity/BargainExportExcel?id=<%=ViewData["HouseId"] %>">
                    导出此列表</a>
            </div>
        </h4>
        <div class="mt15 clearFix" style="position: relative;">
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab1" id="table1">
                ...
                <%if (Model != null && Model.Count() > 0)
                  {
                      int i = 1;
                      foreach (TXModel.NHouseActivies.Bargain.CT_UserList item in Model)
                      { %>
                <tr>
                    <td>
