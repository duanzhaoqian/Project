...
{
    [LoginChecked]
    [ActionFilters.AdminPageInfo(6, (int) AdminNavi.NewHouseMg.Activities.Bargain)]
    public class BargainActiveController : BaseController
    {
        //
        // GET: /BargainActive/
        ...
        public ActionResult SearchResult(PVS_NH_Bargain search, int pageIndex, int pageSize)
        {
            ...
            var list = bargainBll.GetBargainListByPages(search, pageIndex, pageSize);
            return PartialView("PageTables/Activities/_BargainActive", list);
        }
        ...
        {
            var bargainBll = new Bargain_AdminBll();
            int count = bargainBll.GetBargainByPageCounts(search);
        ...
        public ActionResult RegistrationUserShow(int id, int premiseId,int houseId)
        {
            var  _premisesbll = new PremisesBll();
            var premises = _premisesbll.GetPremisesbyId(premiseId);
            ...
            ViewData["endtime"] = new Bargain_AdminBll().GetEntity_ById(id).EndTime;
        ...
        public ActionResult RegistrationUserSearchResult(PVS_NH_RegistrationUser search, int pageIndex, int pageSize)
        {
            ...
            var list = bargainBll.GetUsersListByPages(search, pageIndex, pageSize);
            var models= bargainBll.GetMessByWhere(search.Id);
            ViewData["models"] = models;
            return PartialView("PageTables/Activities/_BargainUserList", list);
        ...
        {
            var bargainBll = new Bargain_AdminBll();
            int count = bargainBll.GetUsersListByPageCounts(search);
            return Json(new {result = count});
        ...
        public string GetHouseInfos(int activeId)
        {
            ...
            {
                var house = houses[0];
                houseInfo = string.Format("{0} {1}{2} {3} {4}层 {5}房-砍价活动报名用户列表", house.PremiseName, house.BuildingName, house.BuildingNameType, house.UnitName, house.FloorString, house.HouseName);
            }
        ...
        {
            var bargainBll = new Bargain_AdminBll();
            var list = bargainBll.GetOutPutRegistUsersByList(search);
            ...
            var listResult = list.ConvertAll(it => new OutputExcel()
            {
                ...
                CellText10 = string.Format("{0:0,0}", it.BidUserPrice)

            });
            const string fileName = "活动名称-砍价活动报名用户列表";
        ...
        /// <returns></returns>
        public ActionResult UpdateBargain(int Id)
        {
            var _bargainBll = new Bargain_AdminBll();
            var _geographyBll = new GeographyBll();
            PVM_NH_BargainActive model = _bargainBll.GetBargainActiviesInfo(Id);
            //楼盘
            ...
            CurrentPageIndex();
            ViewData["backurl"] = string.IsNullOrEmpty(Convert.ToString(Request.Params["backurl"])) ? Url.SiteUrl().BargainActiveIndex : Request.Params["backurl"];
            return View(model);
        }
        ...
        public ActionResult Modify(PVM_NH_Biding model)
        {
            ...
            var result = _bargainBll.UpdateBargain_Admin(model);
            if (result.Code == "1")
            {
                // 发消息
                ...
                ExecuteOperResult(new Z_OperAdminLog
                {
                    ...
                    OperDes = string.Format("新房管理-砍价活动管理-修改活动编号[{0}]", model.ActivitiesId),
                    Type = 0
                });
        ...
        public ActionResult HouseSearchResult(PVS_NH_Biding_House search, int pageIndex, int pageSize)
        {
            ...
            var _bargainBll = new Bargain_AdminBll();
            pageIndex = pageIndex + 1;
            var list = _bargainBll.GetBidingHousePageList_BySearch_Admin(search, pageIndex, pageSize);
        ...
        {
            var _bargainBll = new Bargain_AdminBll();
            int count = _bargainBll.GetBidingHousePageCount_BySearch_Admin(search);
            return Json(new {result = count});
        }
        ...
        {
            var _bargainBll = new Bargain_AdminBll();
            var activity = _bargainBll.GetEntity_ById(id);
            var result = _bargainBll.DeleteActive(id);
            if (result.Code == "1")
            {
                // 发消息
                ...
                ExecuteOperResult(new Z_OperAdminLog
                {
                    ...
                    CreateTime = DateTime.Now,
                    OperIP = Request.UserHostAddress,
                    OperDes = string.Format("新房管理-砍价活动管理-删除活动编号[{0}]", id),
                    Type = 0
                });
        ...
        /// 修改用户活动出价状态
        /// </summary>
        /// <param name="id">BidPrice表Id</param>
        ...
        public JsonResult UpdateStatus(int id, int status)
        {
            ...
            var resultFlag = _bargainBll.EditBidPriceState(id,status);
            if (resultFlag)
            {
                //SendMessage.SendLongHouseIndexMessage("update", id, houseInfo.CityId);   //发送消息
                //管理员操作日志录入
                base.ExecuteOperResult(new Z_OperAdminLog()
                {
                    AdminID = _ServiceContext.CurrentUser.Id,
                    CreateTime = DateTime.Now,
                    OperIP = Request.UserHostAddress,
                    OperDes = string.Format("砍价活动管理-砍价报名用户列表 活动编号{0}：", id)
                });
