...
function dragDropEnd(ev) {
    ...
    if (destinationObject && destinationObject != activeImage) {
        ...
        var activeImageId = activeImage.id;
        var destinationObjectId = destinationObject.id;
        ...
        //将排序结果保存
        SaveSortedImageBox(activeImageId, destinationObjectId);
    }
    return false;
}
...
function initGallery() {
    var divs = document.getElementsByTagName('div');
    for (var no = 0; no < divs.length; no++) {
        if (divs[no].className == 'imageBox_theImage') {
