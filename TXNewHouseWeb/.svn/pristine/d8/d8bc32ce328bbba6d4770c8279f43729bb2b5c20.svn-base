...
(function ($) {
    ...
    //freeui 继承方法
    Function.prototype.freeExtend = function (parent, overrides) {
    ...
    //延时加载
    Function.prototype.freeDefer = function (o, defer, args) {
        var fn = this;
    ...
    window.free = $.freeui = {
        ...
        remove: function (arg) {
            if (typeof arg == "string" || typeof arg == "number") {
                delete $.freeui.managers[arg];
            }
            else if (typeof arg == "object" && arg instanceof $.freeui.core.Component) {
                delete $.freeui.managers[arg.id];
            }
        },
        //获取freeui对象
        //1,传入freeui ID
        ...
        get: function (arg, idAttrName) {
            idAttrName = idAttrName || "freeuiid";
            if (typeof arg == "string" || typeof arg == "number") {
                return $.freeui.managers[arg];
            }
            else if (typeof arg == "object" && arg.length) {
                if (!arg[0][idAttrName] && !$(arg[0]).attr(idAttrName)) return null;
                return $.freeui.managers[arg[0][idAttrName] || $(arg[0]).attr(idAttrName)];
            }
            return null;
        },
        //根据类型查找某一个对象
        ...
        //$.fn.free{Plugin} 和 $.fn.freeGet{Plugin}Manager
        //会调用这个方法,并传入作用域(this)
        //@parm [plugin]  插件名
        //@parm [args] 参数(数组)
        ...
        run: function (plugin, args, ext) {
            if (!plugin) return;
            ext = $.extend({
                defaultsNamespace: 'freeDefaults',
                methodsNamespace: 'freeMethods',
            ...
            plugin = plugin.replace(/^freeGet/, '');
            plugin = plugin.replace(/^free/, '');
            if (this == null || this == window || ext.isStatic) {
                if (!$.freeui.plugins[plugin]) {
                    $.freeui.plugins[plugin] = {
                        fn: $['free' + plugin],
                        isStatic: true
                    };
                }
                return new $.freeui[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!$.freeui.plugins[plugin]) {
                $.freeui.plugins[plugin] = {
                    fn: $.fn['free' + plugin],
            ...
            if (/Manager$/.test(plugin)) return $.freeui.get(this, ext.idAttrName);
            this.each(function () {
                if (this[ext.idAttrName] || $(this).attr(ext.idAttrName)) {
                    var manager = $.freeui.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
                ...
                if (ext.hasElement) {
                    new $.freeui[ext.controlNamespace][plugin](this, p);
                }
                else {
                    new $.freeui[ext.controlNamespace][plugin](p);
                }
            });
            if (this.length == 0) return null;
            if (args.length == 0) return $.freeui.get(this, ext.idAttrName);
            if (typeof args[0] == 'object') return $.freeui.get(this, ext.idAttrName);
            if (typeof args[0] == 'string') {
                var manager = $.freeui.get(this, ext.idAttrName);
    ...
    //关联起来
    $.freeui.defaults = $.freeDefaults;
    $.freeui.methods = $.freeMethos;

    //获取freeui对象
    //@parm [plugin]  插件名,可为空
    $.fn.free = function (plugin) {
        if (plugin) {
            return $.freeui.run.call(this, plugin, arguments);
        }
        else {
            return $.freeui.get(this);
        }
    };
    ...
    $.freeui.core.Component = function (options) {
        //事件容器
    ...
    $.extend($.freeui.core.Component.prototype, {
        __getType: function () {
            return '$.freeui.core.Component';
        },
        __idPrev: function () {
            return 'freeui';
        },
    ...
    //@parm [options] 组件的参数
    $.freeui.core.UIComponent = function (element, options) {
        $.freeui.core.UIComponent.base.constructor.call(this, options);
    ...
    $.freeui.core.UIComponent.freeExtend($.freeui.core.Component, {
        __getType: function () {
            return '$.freeui.core.UIComponent';
        },
        //扩展方法
        ...
        _init: function () {
            ...
            if (!this.element) {
                this.id = this.options.id || $.freeui.getId(this.__idPrev());
            }
            else {
                this.id = this.options.id || this.element.id || $.freeui.getId(this.__idPrev());
            }
            //存入管理器池
            $.freeui.add(this);
            ...
            //读取freeui这个属性，并加载到参数，比如 freeui = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("freeui")) {
                try {
                    var attroptions = $(this.element).attr("freeui");
        ...
        _rendered: function () {
            if (this.element) {
                $(this.element).attr("freeuiid", this.id);
            }
        },
        //返回要转换成freeui参数的属性,比如['url']
        attr: function () {
            return [];
        },
        destroy: function () {
    ...
    //表单控件基类
    $.freeui.controls.Input = function (element, options) {
        $.freeui.controls.Input.base.constructor.call(this, element, options);
    };

    $.freeui.controls.Input.freeExtend($.freeui.core.UIComponent, {
        __getType: function () {
            return '$.freeui.controls.Input';
        },
        attr: function () {
            return ['nullText'];
        },
    ...
    $.freeui.win = {
        ...
        mask: function (win) {
            function setHeight() {
                if (!$.freeui.win.windowMask) return;
                var h = $(window).height() + $(window).scrollTop();
                $.freeui.win.windowMask.height(h);
            }
            if (!this.windowMask) {
                this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.freeuiwin', setHeight);
        ...
        unmask: function (win) {
            ...
            for (var i = 0, l = jwins.length; i < l; i++) {
                var winid = jwins.eq(i).attr("freeuiid");
                if (win && win.id == winid) continue;
                //获取freeui对象
                var winmanager = $.freeui.get(winid);
        ...
        setFront: function (win) {
            var wins = $.freeui.find($.freeui.core.Win);
            for (var i in wins) {
                var w = wins[i];
                if (w == win) {
    ...
    //窗口基类 window、dialog
    $.freeui.core.Win = function (element, options) {
        $.freeui.core.Win.base.constructor.call(this, element, options);
    };

    $.freeui.core.Win.freeExtend($.freeui.core.UIComponent, {
        __getType: function () {
            return '$.freeui.controls.Win';
        },
        mask: function () {
            if (this.options.modal)
                $.freeui.win.mask(this);
        },
        unmask: function () {
            if (this.options.modal)
                $.freeui.win.unmask(this);
        },
        min: function () {
        },
        max: function () {
    ...
    $.freeui.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify : function (o) {
        var f = function (n) {
            return n < 10 ? '0' + n : n;
        },
        ...
        if (type === 'object') {
            ...
            if (o.constructor === Array) {
                for (var i = 0, l = o.length; i < l; i++) {
                    pairs.push($.freeui.toJSON(o[i]) || 'null');
                }
                return '[' + pairs.join(',') + ']';
            }
            var name, val;
