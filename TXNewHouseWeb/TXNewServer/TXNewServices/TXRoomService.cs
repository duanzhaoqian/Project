...
{
    partial class TXRoomService : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            ...
            OptLog.Log("Info：", "TXRoom", "房源户型服务启动：" + DateTime.Now, true);
            //CreateThread();
        ...
        {
            var mqnames = MQHelper.GetRoomQueueNamesInfomation();
        ...
        private List<OperClass> OperDictionary(string key, string value)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(value))
                {
                    ...
                    OptLog.Log("Info：", "TXRoom", "接收消息:处理类型：" + type + "，楼盘ID：" + premisesId + "　城市ID：" + cityid, false);
                    ...
                    while (true)
                    {
                        ...
                        if (opers != null && opers.Count > 0)
                        {
                            ...
                            foreach (var item in opers)
                            {
                                ...
                                {
                                    logInsertMsg += "接收消息并执行方法AddDocumentForRoom，参数楼盘ID： " + item.ID + "，MQName:" + qname + "，城市ID：" + item.CityID;
                                    insertId += item.ID + ",";
                                    OptLog.Log("Info：", "TXPremises", logInsertMsg, true);
                                }
                                else if (mtype.Contains("delete"))
                                {
                                    logDelMsg += "接收消息并执行方法DeleteDocumentForRoom，参数ID： " + item.ID + "，MQName:" + qname + "，城市ID：" + item.CityID;
                                    delId += item.ID + ",";
                            ...
                            if (!string.IsNullOrEmpty(insertId))
                            {
                                ...
                                Opt.AddDocumentForRoom(insertId, qname);
                            }
                            if (!string.IsNullOrEmpty(delId))
                            {
                                delId = delId.TrimEnd(',');
                                string[] ids = delId.Split(',');
                                Opt.DeleteDocumentForRoom(ids, qname);
                            }
