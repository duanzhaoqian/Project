...
namespace Commons
{
    public class LuceneOpt
    {
        ...
        public int AddDocumentForPremiseses(int Id, bool isCreate = true)
        {
            ...
            try
            {
                lock (MyLock)
                {
                    ...
                    {
                        analyzer = new Lucene.Net.Analysis.PanGu.PanGuAnalyzer();// new ProductAnalyzer();//新房分词器
                        //加载楼盘索引分词
                        InitPremisesAnalyzer(analyzer);
        ...
        private Document GetPremisesDocument(IndexPremises premises)
        {
            ...
            //document.Add(new NumericField("PremisesID", Field.Store.YES, true).SetIntValue(Convert.ToInt32(premises.PremisesID)));
        ...
        private void InitPremisesAnalyzer(Analyzer analyzer)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(info.KeyWord))
                    {
                        ...
                        Query q1 = new Lucene.Net.QueryParsers.QueryParser(Lucene.Net.Util.Version.LUCENE_29, "PremisesName", analyzer).Parse(info.KeyWord);
                        Query q2 = new Lucene.Net.QueryParsers.QueryParser(Lucene.Net.Util.Version.LUCENE_29, "PremisesAddress", analyzer).Parse(info.KeyWord);
