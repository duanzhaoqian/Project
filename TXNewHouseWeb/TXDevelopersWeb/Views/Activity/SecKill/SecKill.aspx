...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btn_sure02").click(function () {
                ...
                //var bstart = $("#baomingStart"); //减价幅度
                //var bend = $("#baomingEnd"); //最大幅度
                ...
                if (Verify.IsEmpty(bidPrice.val())) return Verify.Error("秒杀价格不能为空", bidPrice.next());
                ...
                //if (!Verify.IsDate(bstart.val())) return Verify.Error("日期不能为空", bend);
                //if (!Verify.IsDate(bend.val())) return Verify.Error("日期不能为空", bend);
                //if (!Verify.Compare(bstart.val(), bend.val())) return Verify.Error("日期不正确", bend);

                if (Verify.IsEmpty(start.val())) return Verify.Error("日期不能为空", end);
                if (Verify.IsEmpty(end.val())) return Verify.Error("日期不能为空", end);
                ...
                //                $("#td_baoming01").hide();
                //                $("#td_baoming02").show();
                //                $("#span_baomingStart02").html(bstart.val());
                //                $("#span_baomingEnd02").html(bend.val());
        ...
        function QueryHouse() {
            ...
            //var saleState = $(".selbox span").eq(3).attr("data-salsestate");  //状态
            var saleState = 2;  //状态
            jQuery.ajax({ type: "get", async: false, url: mainUrl + "/Activity/SecKill", data: { premisesId: premisesId, buildId: buildId, unitId: unitId, floorId: floorId, saleState: saleState, m: Math.random() }, cache: false,
                success: function (data) {
                    $("#divpages").html(data);
                }
            });
        ...
        function Complete() {
            ...
            if (Verify.IsEmpty(bidPrice.val())) return Verify.Error("秒杀价格不能为空", bidPrice.next());
            if (!Verify.Integer2(bidPrice.val())) return Verify.Error("秒杀价格输入不正确", bidPrice.next());
            if (Verify.IsEmpty(bond.val())) return Verify.Error("活动保证金不能为空", bond.next());
            if (!Verify.Integer(bond.val())) return Verify.Error("活动保证金输入不正确", bond.next());
            ...
            $.ajax({
                url: "/Activity/CreateSecKill",
                type: 'post',
                cache: false,
                dataType: "json",
                async: false,
                data: { cityId: cityId, premisesId: premisesId, bidPrice: bidPrice.val(), bond: bond.val(), start: start.val(), end: end.val(), Ids: house, activState: activState },
                success: function (data) {
                    if (data.sucess != "0") {
                        window.location = "<%=TXCommons.GetConfig.BaseUrl%>" + "Activity/SecKillList";
                        //                        ShowCondition();
                        //                        QueryHouse();
    ...
    <div class="content">
        <h4 class="title_h4 mb10">
            <span>创建秒杀</span><em class="ts_right"><a href="<%=TXCommons.GetConfig.BaseUrl%>Activity/SecKillList">返回秒杀房源列表 </a></em>
        </h4>
        ...
        <td id="td_maxmargin02" style="display:none"><span id="span_maxmargin"></span>元</td>
            ...
            <tr>
                ...
                <td id="td_bond02" style="display: none">
                    <span id="span_bond02"></span><span class="ie_valign_5" style="display: none">元</span>
                </td>
            </tr>
            <tr style="display: none">
                ...
                <td id="td_baoming01">
                    <span class="ie_valign_5">开始</span><input id="baomingStart" type="text" class="input_wauto w150 ml5 mr20" onclick="WdatePicker()" readonly="readonly" /><span class="ie_valign_5">结束</span><input type="text" id="baomingEnd" class="input_wauto w150 ml10" onclick="WdatePicker()" readonly="readonly" />
                </td>
                <%--隐藏/显示：时间--%>
            ...
            <tr>
                ...
                <td id="td_date01">
                    ...
                    <input id="txtStart" class="input_wauto w150 ml10" type="text" onfocus="WdatePicker({dateFmt:'yyyy-M-d HH:00:00',minDate:'%y-%M-%d'})" readonly="readonly" />
                    <span class="ie_valign_5">结束</span>
                    <input id="txtEnd" type="text" class="input_wauto w150 ml10" onfocus="WdatePicker({dateFmt:'yyyy-M-d H:00:00',minDate:'#F{$dp.$D(\'txtStart\',{H:1})}',maxDate:'#F{$dp.$D(\'txtStart\',{d:29})}' })" readonly="readonly" />
                </td>
                <%--隐藏/显示：时间--%>
