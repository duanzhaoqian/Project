...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $("#btn_sure02").click(function () {
                ...
                var addPrice = $("#txtAddPrice"); //减价幅度
                var maxPrice = $("#txtMaxPrice"); //最大幅度
                var bond = $("#txtBond"); //证金金额
                var start = $("#txtStart"); //竞拍开始时间
                var end = $("#txtEnd"); //竞拍结束

                bidPrice.next().next().remove();
                addPrice.next().next().remove();
                maxPrice.next().next().remove();
                bond.next().next().remove();
                end.next().remove();

                if (Verify.IsEmpty(bidPrice.val())) return Verify.Error("起拍价格不能为空", bidPrice.next());
                if (!Verify.Integer2(bidPrice.val())) return Verify.Error("最多可输入9位正数", bidPrice.next());
                if (Verify.IsEmpty(addPrice.val())) return Verify.Error("减价幅度不能为空", addPrice.next());
                if (!Verify.Integer2(addPrice.val())) return Verify.Error("最多可输入9位正数", addPrice.next());
                if (Verify.IsEmpty(maxPrice.val())) return Verify.Error("最大幅度不能为空", maxPrice.next());
                if (!Verify.Integer2(maxPrice.val())) return Verify.Error("最多可输入9位正数", maxPrice.next());
                ...
                $("#td_addmargin01").hide();
                $("#td_addmargin02").show();
                $("#span_addmargin02").html(addPrice.val());

                $("#td_maxmargin01").hide();
                $("#td_maxmargin02").show();
                $("#span_maxmargin").html(maxPrice.val());
        ...
        function QueryHouse() {
            ...
            //var saleState = $(".selbox span").eq(3).attr("data-salsestate");  //状态
            var saleState = 2;  //状态
            jQuery.ajax({ type: "get", async: false, url: mainUrl + "/Activity/Bargain", data: { premisesId: premisesId, buildId: buildId, unitId: unitId, floorId: floorId, saleState: saleState, m: Math.random() }, cache: false,
                success: function (data) {
                    $("#divpages").html(data);
                }
            });
        ...
        function Complete() {
            ...
            $.ajax({
                url: "/Activity/CreateBiding",
                type: 'post',
                cache: false,
                dataType: "json",
                async: false,
                data: { cityId: cityId, premisesId: premisesId, bidPrice: bidPrice.val(), addPrice: addPrice.val(), maxPrice: maxPrice.val(), bond: bond.val(), start: start.val(), end: end.val(), Ids: house, activState: activState },
                success: function (data) {
                    if (data.sucess != "0") {
                        window.location = "<%=TXCommons.GetConfig.BaseUrl%>" + "Activity/BidingList";
                        //                        ShowCondition();
                        //                        QueryHouse();
    ...
    <div class="content">
        <h4 class="title_h4 mb10">
            <span>创建竞价</span><em class="ts_right"><a href="<%=TXCommons.GetConfig.BaseUrl%>Activity/BidingList">返回竞价房源列表 </a></em>
        </h4>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_box1" id="table_condition">
            <tr>
                <th scope="row" width="100" align="right">
                ...
                <td>
                   ...
                   <a>2层</a>--%>
                        ...
                        <div class="selbox mr10" style="display: none">
                            <span class="pl10" data-salsestate="0">--选择状态--</span>
                            <%--<span class="pl10" data-salsestate="-1">--选择状态--</span>--%>
                            <div style="display: none;" class="sel_list">
                                <a data-salsestate="-1">--选择状态--</a> <a data-salsestate="0">未售</a> <a data-salsestate="1">在售</a> <a data-salsestate="2">售罄</a>
                            </div>
                        </div>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_box1" id="table_active">
            <tr>
                <th scope="row" width="100" align="right">
                    起拍价格：
                </th>
            ...
            <tr>
                <th scope="row" width="150" align="right">
                    加价幅度：
                </th>
                <td id="td_addmargin01">
                    必须为<input type="text" class="input_wauto w180 mr10" id="txtAddPrice" /><span>万元 的整数倍</span>
                </td>
                <td id="td_addmargin02" style="display: none">
                    <span id="span_addmargin02"></span>万元的整数倍
                </td>
            </tr>
            <tr>
                <th scope="row" width="150" align="right">
                ...
                <td id="td_maxmargin01">
                    单次加价不高于<input type="text" class="input_wauto w180 mr10" id="txtMaxPrice" /><span>万元</span>
                </td>
                <td id="td_maxmargin02" style="display: none">
                    <span id="span_maxmargin"></span>万元
                </td>
            </tr>
            <tr>
                <th scope="row" width="100" align="right">
                ...
                <td id="td_bond01">
                    <input type="text" class="input_wauto w180 mr10" id="txtBond" /><span class="ie_valign_5" id="span_yuan">元</span>
                </td>
                <%--隐藏/显示：活动保证金金额--%>
            ...
            <tr>
                ...
                <td id="td_date01">
                    ...
                    <input id="txtStart" type="text" class="input_wauto w150 ml5 mr20" onfocus="WdatePicker({dateFmt:'yyyy-M-d',minDate:'%y-%M-%d'})" readonly="readonly" />
                    <span class="ie_valign_5">结束</span>
                    <input type="text" id="txtEnd" class="input_wauto w150 ml10" onfocus="WdatePicker({dateFmt:'yyyy-M-d',minDate:'#F{$dp.$D(\'txtStart\',{d:0})}',maxDate:'#F{$dp.$D(\'txtStart\',{d:29})}' })" readonly="readonly" />
                </td>
                <%--隐藏/显示：时间--%>
                <td id="td_date02" style="display: none">
                    <span class="ie_valign_5">开始</span><span id="span_start02"></span><span class="ie_valign_5">结束</span><span id="span_end02"></span>
                </td>
            </tr>
            <tr>
                <td colspan="2">
