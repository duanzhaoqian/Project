...
namespace KYJ.ZS.DAL.DB
{
    public class ToolHelper
    {
        ...
        #endregion
        #region 返回DataSet对象 作者：邓福伟 时间：2014-6-23
        /// <summary>
        /// 传入SQL语句，返回DataTable对象
        /// </summary>
        /// <param name="sql"></param>
        /// <returns></returns>
        public DataSet GetDataSet(string sql, SqlParam param)
        {
            if (string.IsNullOrEmpty(sql))
            {
                return new DataSet();
        ...
        /// </summary>
        /// <param name="sql"></param>
        public int SqlExecuteRuturnId(string sql)
        {
            return Auxiliary.ToInt32(SqlExecuteRuturnId(sql, null));
        }
        public int SqlExecuteRuturnId(string sql, SqlParam param)
        {
            if (string.IsNullOrEmpty(sql))
            {
                return 0;
            ...
            try
            {
                ...
                if (param != null)
                    foreach (SqlParameter p in param.Paramters)
                        cmd.Parameters.Add(p);
                cmd.CommandTimeout = 300;
                return Auxiliary.ToInt32(cmd.ExecuteScalar());
            }
            catch (System.Exception e)
            {
                throw new Exception(e.Message + sql);
        ...
        public DataTable GetPages(int index, string where_, string OrderByColumn, string ColumnList, string TableList, int PageSize, bool IsAccount,out int TotalRecord, out int TotalPage)
        {
            ...
            SqlParameter[] parm = { 
                                  ...
                                  new SqlParameter("@ColumnList",SqlDbType.NVarChar,1000),
