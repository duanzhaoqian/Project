...
namespace KYJ.ZS.Pay.Controllers
{
    ...
    public class PaymentController : BaseController
    {
        ...
        public ActionResult Order(string ordernum, string callbackurl)
        {
            var _paymentBll = new KYJ.ZS.BLL.Payments.PaymentBll();
            //当前登录 用户ID
            var uid = _ServiceContext.CurrentUser.UserID;
            var entity = _paymentBll.GetPaymentByOrderNum(ordernum, uid);
        ...
        [HttpGet]
        [KYJ.ZS.Pay.Controllers.ActionFilters.LoginChecked]
        public ActionResult Verify(string ordernum, BankType mode, string callbackurl)
        {
            var _paymentBll = new KYJ.ZS.BLL.Payments.PaymentBll();
            //当前登录 用户ID
            var uid = _ServiceContext.CurrentUser.UserID;
        ...
        [KYJ.ZS.Pay.Controllers.ActionFilters.LoginChecked]
        [HttpGet]
        public ActionResult Send(string ordernum, string mode)
        {
            ...
            var uid = _ServiceContext.CurrentUser.UserID;
            //获取当前登录用户指定订单号的支付信息
            var order = _paymentBll.GetPaymentByOrderNum(ordernum, uid);
            ...
            string bgUrl = PubConstant.PayBaseUrl + string.Format("/payment/receive.html?ordernum={0}&uid={1}&mid={2}&pprice={3}", ordernum, uid, order.MerchantID, pprice);//Url.PaySiteUrl().Receive(ordernum, status, callbackurl);//PubConstant.BaseUrl + "Landlord/receive?id=" + ordernum.ToString() + "&uid=" + this.GetUserInfo.Id + "&type=" + status + "&pprice=" + pprice;
            //网关版本，固定值：v2.0,该参数必填。
            string version = "v2.0";
        ...
        public ContentResult Receive()
        {
            ...
            if (f.VerifySignature(result, SignatureByte))
            {
                ...
                if (payResult == "10")
                {
                    ...
                    var r = _payBll.UpdateOrderStatus(ordernum, uid);

                    if (r > 0)
                    {
                        _payBll.MerhantAmount(mid, Auxiliary.ToDecimal(pprice), 3, 2, "用户订单支付金额" + pprice + "元");
                        issuccess = true;
                    }
