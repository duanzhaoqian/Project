namespace KYJ.ZS.Search.Api
{
    ...
    public class LuceneOpt
    {
        ...
        public int AddDocForRentalGoodses(string id)
        {
            ...
            try
            {
                var table = this.searchDal.GetRentalGoodses(id);
                if (table != null && table.Rows.Count > 0)
                {
                    analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_29);
                    bool isCreate = string.IsNullOrEmpty(id);
                    directory = FSDirectory.Open(new DirectoryInfo(this.RentalGoodsesIndexPath));
                    ...
                    for (int k = 0; k < table.Rows.Count; k++)
                    {
                        ...
                        {
                            Console.WriteLine("id:" + idstr);
                            Document document = this.GetRentalGoodsesDocument(dr);
                        ...
                        {
                            Commons.Tool.Log("添加出租商品索引异常", "HouseID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
                        }
            ...
            {
                Commons.Tool.Log("添加出租商品索引异常", "GoodsID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
            }
            finally
            {
                if (analyzer != null)
        ...
        public int AddDocForSaleGoodses(string id)
        {
            ...
            try
            {
                var table = this.searchDal.GetSaleGoodses(id);
                if (table != null && table.Rows.Count > 0)
                {
                    analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_29);
                    bool isCreate = string.IsNullOrEmpty(id);
                    directory = FSDirectory.Open(new DirectoryInfo(this.SaleGoodsesIndexPath));
                    indexWriter = new IndexWriter(directory, false, analyzer, isCreate);
                    indexWriter.SetMaxFieldLength(15000);
                    for (int k = 0; k < table.Rows.Count; k++)
                    {
                        ...
                        {
                            Console.WriteLine("id:" + idstr);
                            Document document = this.GetSaleGoodsesDocument(dr);
                        ...
                        {
                           Commons.Tool.Log("添加出售商品索引异常", "HouseID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
                        }
            ...
            {
                Commons.Tool.Log("添加出售商品索引异常", "GoodsID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
            }
            finally
            {
                if (analyzer != null)
        ...
        public int AddDocForBrands(string id)
        {
            ...
            try
            {
                DataTable table = this.searchDal.GetBrands(id);
                if (table != null  && table.Rows.Count > 0)
                {
                    analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_29);
                    bool isCreate = string.IsNullOrEmpty(id);
                    directory = FSDirectory.Open(new DirectoryInfo(this.BrandsIndexPath));
                    indexWriter = new IndexWriter(directory, false, analyzer, isCreate);
                    indexWriter.SetMaxFieldLength(15000);
                    ...
                    for (int k = 0; k < table.Rows.Count; k++)
                    {
                        ...
                        {
                            Console.WriteLine("id:" + idstr);
                            Lucene.Net.Documents.Document document = this.GetBrandsDocument(dr);
                        ...
                        {
                            KYJ.ZS.Commons.Tool.Log("添加品牌索引异常", "HouseID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
                        }
                    }
                    KYJ.ZS.Commons.Tool.Log("添加品牌索引完成", "count:" + count, this.LogPath);
                    return count;
                }
            }
            catch (Exception ex)
        ...
        Lucene.Net.Documents.Document GetBrandsDocument(DataRow dr)
        {
            ...
            DataTable table = this.searchDal.GetCategoryBrands(Id);
        ...
        public  int  AddDocForAttributes(string id)
        {
            ...
            try
            {
                DataTable table = this.searchDal.GetAttributes(id);
                if (table != null && table.Rows.Count > 0)
                {
                    analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_29);
                    bool isCreate = string.IsNullOrEmpty(id);
                    directory = FSDirectory.Open(new DirectoryInfo(this.AttributesIndexPath));
                    indexWriter = new IndexWriter(directory, false, analyzer, isCreate);
                    indexWriter.SetMaxFieldLength(15000);
                    ...
                    for (int k = 0; k < table.Rows.Count; k++)
                    {
                        ...
                        try
                        {
                            Lucene.Net.Documents.Document document = this.GetAttributesDocument(dr);
                        ...
                        {
                            KYJ.ZS.Commons.Tool.Log("添加属性索引异常", "ID:" + idstr + "索引添加错误：" + ex.Message, this.LogPath);
                        }
                    }
                    KYJ.ZS.Commons.Tool.Log("添加属性索引完成", "count:" + count, this.LogPath);
                    return count;
                }
            }
            catch (Exception ex)
        ...
        Lucene.Net.Documents.Document GetAttributesDocument(DataRow dr)
        {
            ...
            DataTable table = this.searchDal.GetCategoryAttributes(Id);
            StringBuilder strs = new StringBuilder("");
            if (table != null && table.Rows.Count > 0)
            {
        ...
        public int AddDocForCategories(string id)
        {
            ...
            try
            {
                DataTable table = this.searchDal.GetCategories();
                if (table != null && table.Rows.Count > 0)
                {
                    analyzer = new StandardAnalyzer(Lucene.Net.Util.Version.LUCENE_29);
                    bool isCreate = string.IsNullOrEmpty(id);
                    directory = FSDirectory.Open(new DirectoryInfo(this.CategoriesIndexPath));
                    indexWriter = new IndexWriter(directory, false, analyzer, isCreate);
                    indexWriter.SetMaxFieldLength(15000);
                    ...
                    for (int k = 0; k < table.Rows.Count; k++)
                    {
                        ...
                        try
                        {
                            Lucene.Net.Documents.Document document = this.GetCategoriesDocument(dr);
