...
namespace KYJ.ZS.Pay.Controllers
{
    ...
    public class TenancyController : BaseController
    {
        ...
        public ActionResult Order(string ordernum, int type, string callbackurl)
        {
            ...
            {
                case 1:
                    entity = _paymentBll.GetTenancyByOrderNum(ordernum, uid);
                    break;
                case 2:
                    entity = _paymentBll.GetPayoutByOrderNum(ordernum, uid);
                    break;
                default:
                    entity = _paymentBll.GetTenancyByOrderNum(ordernum, uid);
                    break;
        ...
        public ActionResult Send(string ordernum, int type, string mode, string callbackurl)
        {
            ...
            var uid = _ServiceContext.CurrentUser.UserID;
            //验证当前登录用户的订单是否需要需求
            if (!_paymentBll.IsRelet(ordernum, uid))
            {
                return Redirect(Url.UserSiteUrl().Index);
            }
            //获取当前登录用户指定订单号的支付信息
            PaymentEntity order = null;
            switch (type)
            {
                case 1:
                    order = _paymentBll.GetPaymentByOrderNum(ordernum, uid);
                    break;
                case 2:
                    order = _paymentBll.GetPayoutByOrderNum(ordernum, uid);
                    break;
                default:
                    order = _paymentBll.GetPaymentByOrderNum(ordernum, uid);
                    break;
            ...
            if (order != null)
            {
                pprice = order.MonthPrice.ToString("#0.00");
            }
            else
            {
                return Redirect(Url.UserSiteUrl().Index);
            ...
            string bgUrl = PubConstant.PayBaseUrl + string.Format("/tenancy/receive.html?ordernum={0}&uid={1}&mid={2}&pprice={3}&type={4}&callbackurl={5}", ordernum, uid, order.MerchantID, pprice, type, callbackurl);//Url.PaySiteUrl().Receive(ordernum, status, callbackurl);//PubConstant.BaseUrl + "Landlord/receive?id=" + ordernum.ToString() + "&uid=" + this.GetUserInfo.Id + "&type=" + status + "&pprice=" + pprice;
            //网关版本，固定值：v2.0,该参数必填。
            string version = "v2.0";
        ...
        public ContentResult Receive()
        {
            ...
            if (f.VerifySignature(result, SignatureByte))
            {
                ...
                if (payResult == "10")
                {
                    ...
                    {
                        case 1:
                            if (_payBll.IsRelet(ordernum, uid))
                            {
                                var r = _payBll.OrderRelet(ordernum, uid);
                                if (r > 0)
                                {
                                    issuccess = true;
                                    _payBll.MerhantAmount(mid, Auxiliary.ToDecimal(pprice), 3, 2, "用户订单#" + ordernum + "#续租金额" + pprice + "元");
                                }
                            }
                            else
                                issuccess = true;
                        ...
                        case 2:
                            if (_payBll.IsRelet(ordernum, uid))
                            {
                                var r = _payBll.OrderBuyout(ordernum, uid);
                                if (r > 0)
                                {
                                    issuccess = true;
                                    _payBll.MerhantAmount(mid, Auxiliary.ToDecimal(pprice), 3, 2, "用户订单#" + ordernum + "#购买金额" + pprice + "元");
                                }
                            }
                            else
                                issuccess = true;
