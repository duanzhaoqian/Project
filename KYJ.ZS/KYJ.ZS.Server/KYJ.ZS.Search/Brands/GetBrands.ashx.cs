...
namespace TXSearch.Brands
{
    /// <summary>
    /// GetBrands 的摘要说明
    /// </summary>
    public class GetBrands : IHttpHandler
    {
        ...
        public List<IndexResult> GetList(IndexSearcher indexSearcher, BooleanQuery bq, Sort sort, out int count)
        {
            ...
            if (hits.Length > 0 && bi < count)
            {
                for (int i = bi; i < ei; i++)
                {
                    ...
                    result.Id = indexSearcher.Doc(doc.doc).Get("brand_id");
                    result.Name = indexSearcher.Doc(doc.doc).Get("brand_name");
