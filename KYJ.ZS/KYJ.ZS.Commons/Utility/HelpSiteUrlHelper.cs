...
namespace KYJ.ZS.Commons
{
    ...
    /// Desc  :帮助中心URLHelper
    /// </summary>
    public class HelpSiteUrlHelper
    {
        private static readonly HelpSiteUrlHelper instance = new HelpSiteUrlHelper();

        private static string BaseUrl;

        internal static HelpSiteUrlHelper Instance
        {
            get
            {
                if (string.IsNullOrEmpty(BaseUrl))
                    BaseUrl = PubConstant.HelpBaseUrl;
                return instance;
            }
        }

        private HelpSiteUrlHelper()
        {
        }

        internal UrlHelper UrlHelper { get; private set; }

        internal HelpSiteUrlHelper SetContext(UrlHelper context)
        {
            if (context == null)
