...
namespace Mongo
{
    public sealed class MongoHelper : IMongo
    {
        ...
        public MongoHelper(string serverconnstr)
        {
            ...
            this.server = new MongoClient(ServerConnstr).GetServer();
        ...
        public WriteConcernResult Delete(string collectionName, string _id)
        {
            ObjectId id;
        ...
        public WriteConcernResult DeleteAll(string collectionName)
        {
            return this.DeleteAll(collectionName, null);
        }

        public WriteConcernResult DeleteAll(string collectionName, IMongoQuery query)
        {
            using (this.server.RequestStart(this.database))
            {
                MongoCollection<BsonDocument> collection = this.database.GetCollection<BsonDocument>(collectionName);
                ...
                return collection.Remove(query,WriteConcern.Acknowledged);
            }
        }
        ...
        public IEnumerable<WriteConcernResult> InsertAll<T>(string collectionName, IEnumerable<T> entitys)
        {
            if (entitys == null)
            {
                return null;
        ...
        public bool InsertOneSafe<T>(string collectionName, T entity)
        {
            ...
            try
            {
                ...
                else
                {
                    using (this.server.RequestStart(this.database))
                    {
                        this.database.GetCollection<BsonDocument>(collectionName).Insert<T>(entity, WriteConcern.Acknowledged);
                        flag = true;
                    }
                }
            }
        ...
        public WriteConcernResult UpdateAll<T>(string collectionName, IMongoQuery query, IMongoUpdate update)
        {
        ...
        public WriteConcernResult UpdateOne<T>(string collectionName, T entity)
        {
            using (this.server.RequestStart(this.database))
            {
