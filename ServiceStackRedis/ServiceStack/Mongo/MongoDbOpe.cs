...
namespace ServiceStack.Mongo
{
    public sealed class MongoDbOpe
    {
        ...
        private static MongoServer server1 = MongoServer.Create(ServerConnstr);
        ...
        {
            MongoServer server = MongoServer.Create(ServerConnstr);
            MongoDatabase database = server.GetDatabase(DatabaseName);
            using (server.RequestStart(database))
            {
        ...
        private static SafeModeResult UpdateOne<T>(string connectionString, string databaseName, string collectionName, T entity)
        {
            MongoServer server = MongoServer.Create(connectionString);
            MongoDatabase database = server.GetDatabase(databaseName);
            using (server.RequestStart(database))
